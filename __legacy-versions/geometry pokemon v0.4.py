...
MAX_JUMP_STRENGTH = 45  # Updated maximum jump strength is now 45
...
class Player:
    def __init__(self):
        ...
        self.vel_y = 0
        self.on_ground = True
        # For variable jump strength (ground jump)
        self.charging = False
        self.jump_charge = 0
        # Allow one extra double jump mid-air (only via X button)
...
def run_game():
    ...
    safe_gap_min = 50
    safe_gap_max = 140  # Horizontal gap less than about 155 pixels
    ...
    # Star coin variables
    star_coins = []         # List of coins in the level
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            # Keyboard input:
            # For jump on ground using space bar (charge jump)
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    if player.on_ground:
                        player.charging = True
                        player.jump_charge = MIN_JUMP_STRENGTH
                # For double jump using the 'x' key when mid-air
            ...
            # Joystick input:
            # A button (assumed button index 0) for initial jump charge when on ground
            if event.type == pygame.JOYBUTTONDOWN:
                if event.button == 0:
                    ...
                    if player.on_ground:
                        player.charging = True
                        player.jump_charge = MIN_JUMP_STRENGTH
                # X button (assumed button index 2) for double jump mid-air
        ...
        # Increase jump charge if jump button is held (keyboard or joystick) and player is on ground
        ...
        # Move platforms, obstacles, and coins
        for platform in platforms[:]:
            platform.move()
        ...
        for coin in star_coins[:]:
            coin.x -= SPEED  # Coins move along with platforms
        ...
        while platforms and platforms[-1].x + platforms[-1].width < WIDTH:
            ...
            if coins_spawned < 3 and random.random() < 0.3:
                ...
                coin_y = new_platform.y - 50  # Place coin above platform
                star_coins.append(StarCoin(coin_x, coin_y))
                coins_spawned += 1
            # Possibly spawn obstacles on the new platform
            if random.random() < 0.5:
                for _ in range(random.randint(1, 2)):
                    obstacles.append(Obstacle(new_platform))
        
        # Update player movement (including collisions with platforms)
        ...
        # Check if player hits deadly spikes
        if player.y + player.height >= HEIGHT - SPIKE_HEIGHT:
            running = False  # Game over

        # End level when time is up
        if remaining_time <= 0:
            running = False
